...
from SAMPLE_TC_PRESSURE import TC_pressure_original
...
for year in range(0,total_years):
    ...
    if storms_per_year>0:
            ...
            TC_data=TC_pressure_original(basin,latlist,lonlist,landfalllist,year,storms_per_year,genesis_month,TC_data)

TC_data=np.array(TC_data)
np.savetxt(os.path.join(output_dir,'STORM_DATA_IBTRACS_'+str(basin)+'_'+str(total_years)+'_YEARS_'+str(loop)+'_Original.txt'),TC_data,fmt='%5s',delimiter=',')
