...
from SAMPLE_TC_PRESSURE import TC_pressure_original_epsilon_plus2
...
basin='WP'
loop=20 #ranges between 0 and 9 to simulate slices of 1000 years

total_years=50 #set the total number of years you'd like to simulate
...
for year in range(0,total_years):
    ...
    if storms_per_year>0:
            ...
            TC_data=TC_pressure_original_epsilon_plus2(basin,latlist,lonlist,landfalllist,year,storms_per_year,genesis_month,TC_data)

TC_data=np.array(TC_data)
np.savetxt(os.path.join(output_dir,'STORM_DATA_IBTRACS_'+str(basin)+'_'+str(total_years)+'_YEARS_'+str(loop)+ 'ORIGINAL_FIXED_EPSILON.txt'),TC_data,fmt='%5s',delimiter=',')
