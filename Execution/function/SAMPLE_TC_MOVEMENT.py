...
original_dir = os.path.join('..', '..', 'Preprocessing', 'original_data')
...
def TC_movement(lon_genesis_list,lat_genesis_list,basin): 
    ...
    constants_all=np.load(os.path.join(input_dir,'TRACK_COEFFICIENTS.npy'),allow_pickle=True,encoding='latin1').item()
    land_mask=np.loadtxt(os.path.join(original_dir, 'Land_ocean_mask_' + str(basin) + '.txt'))
