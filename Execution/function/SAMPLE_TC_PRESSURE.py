...
input_dir = os.path.join('..', '..', 'Preprocessing', 'output_data')
...
def add_parameters_to_TC_data(pressure_list,wind_list,latfull,lonfull,year,storm_number,month,basin,landfallfull,lijst,TC_data,idx):
    ...
    for l in range(0,x):
        if landfallfull[l]==1.:
            ...
            distance=distance_from_coast(lonfull[l],latfull[l],(os.path.join("..", "original_data",'coastal_basemap_data.npy')))
...
def TC_pressure(basin,latlist,lonlist,landfalllist,year,storms,monthlist,TC_data):  
    ...
    JM_pressure=np.load(os.path.join(input_dir,'COEFFICIENTS_JM_PRESSURE.npy'), allow_pickle=True).item()
...
def TC_pressure_epsilon_plus2(basin,latlist,lonlist,landfalllist,year,storms,monthlist,TC_data):  
    """
...
def TC_pressure_original(basin,latlist,lonlist,landfalllist,year,storms,monthlist,TC_data):  
    """
    Calculate TC pressure
