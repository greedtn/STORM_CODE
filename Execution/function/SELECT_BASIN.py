...
def Genesis_month(idx,storms):
    ...
    """
    monthlist=np.load(os.path.join(input_dir,'GENESIS_MONTHS.npy'), allow_pickle=True).item()
...
def Storms(idx): 
    ...
    """
    mu_list=np.loadtxt(os.path.join(input_dir,'POISSON_GENESIS_PARAMETERS.txt'))
